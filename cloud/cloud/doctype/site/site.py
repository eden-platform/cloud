...
from cloud.api.server import prometheus_query

from cloud.exceptions import CannotChangePlan, InsufficientSpaceOnServer
from cloud.marketplace.doctype.marketplace_app_plan.marketplace_app_plan import (
	MarketplaceAppPlan,
)
...
from cloud.api.site import check_dns, get_updates_between_current_and_next_apps
from cloud.overrides import get_permission_query_conditions_for_doctype
from cloud.cloud.doctype.marketplace_app.marketplace_app import (
	get_plans_for_app,
	marketplace_app_hook,
)
from cloud.cloud.doctype.server.server import is_dedicated_server
from cloud.cloud.doctype.site_plan.site_plan import get_plan_config
from cloud.cloud.doctype.site_activity.site_activity import log_site_activity
from cloud.cloud.doctype.site_analytics.site_analytics import create_site_analytics
from cloud.cloud.doctype.resource_tag.tag_helpers import TagHelpers
...
from cloud.utils.dns import _change_dns_record, create_dns_record
...
class Site(Document, TagHelpers):
	...
	if TYPE_CHECKING:
		...
		from cloud.cloud.doctype.resource_tag.resource_tag import ResourceTag
		from cloud.cloud.doctype.site_app.site_app import SiteApp
		from cloud.cloud.doctype.site_config.site_config import SiteConfig
	...
	def set_plan(self, plan):
		from cloud.api.site import validate_plan
	...
	def current_usage(self):
		from cloud.api.analytics import get_current_cpu_usage
...
def process_rename_site_job_update(job):
	...
	if "Success" == first == second:
		...
		job.reload()
		updated_status = "Active"
		from cloud.cloud.doctype.site.pool import create as create_pooled_sites

...
from cloud.cloud.doctype.site_backup.site_backup import (
	SiteBackup,
	process_backup_site_job_update,
)
from cloud.utils import log_error
from cloud.utils.dns import create_dns_record
...
class SiteMigration(Document):
	...
	if TYPE_CHECKING:
		...
		from cloud.cloud.doctype.site_migration_step.site_migration_step import (
			SiteMigrationStep,
		)
	...
	def restore_site_on_destination_server(self):
		...
		site.remote_public_file = backup.remote_public_file
		site.remote_private_file = backup.remote_private_file
		site.remote_config_file = ""  # Use site config from cloud only
		site.bench = self.destination_bench

...
class SSHCertificateAuthority(Document):
	...
	def sign(
		...
		self.run(
			f"ssh-keygen -s ca -t rsa-sha2-512 -I {identity} {host_flag}"
			f" {principals_argument} -z {serial_number} -O no-port-forwarding -O"
			" no-user-rc -O no-x11-forwarding -O no-agent-forwarding -O permit-pty -V"
			f" {duration} {public_key_path}",

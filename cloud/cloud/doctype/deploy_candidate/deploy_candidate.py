...
from cloud.cloud.doctype.app_release.app_release import (
	AppReleasePair,
	get_changed_files_between_hashes,
)
from cloud.cloud.doctype.deploy_candidate.deploy_notifications import (
	create_build_failed_notification,
)
from cloud.cloud.doctype.deploy_candidate.docker_output_parsers import (
	DockerBuildOutputParser,
	UploadStepUpdater,
)
from cloud.cloud.doctype.deploy_candidate.utils import (
	PackageManagerFiles,
	get_package_manager_files,
	load_pyproject,
)
from cloud.cloud.doctype.deploy_candidate.validations import PreBuildValidations
from cloud.cloud.doctype.release_group.release_group import ReleaseGroup
from cloud.utils import get_current_team, log_error, reconnect_on_failure
from cloud.utils.jobs import get_background_jobs, stop_background_job
...
if typing.TYPE_CHECKING:
	...
	from cloud.cloud.doctype.app_release.app_release import AppRelease
...
class DeployCandidate(Document):
	...
	if TYPE_CHECKING:
		...
		from cloud.cloud.doctype.deploy_candidate_app.deploy_candidate_app import (
			DeployCandidateApp,
		)
		from cloud.cloud.doctype.deploy_candidate_build_step.deploy_candidate_build_step import (
			DeployCandidateBuildStep,
		)
		from cloud.cloud.doctype.deploy_candidate_dependency.deploy_candidate_dependency import (
			DeployCandidateDependency,
		)
		from cloud.cloud.doctype.deploy_candidate_package.deploy_candidate_package import (
			DeployCandidatePackage,
		)
		from cloud.cloud.doctype.deploy_candidate_variable.deploy_candidate_variable import (
	...
	def _get_build_server(self):
		...
		if not server:
			server = frappe.get_value("Cloud Settings", None, "build_server")
		return server
...
def should_build_retry_exc(exc: Exception):
	...
	# Redis refused connection (cloud side)

...
from cloud.agent import Agent, AgentCallbackException
from cloud.api.client import is_owned_by_team
...
def process_job_updates(job_name, response_data: "Optional[dict]" = None):
	...
	try:
		from cloud.cloud.doctype.app_patch.app_patch import AppPatch
		from cloud.cloud.doctype.bench.bench import (
		...
		from cloud.cloud.doctype.code_server.code_server import (
		...
		from cloud.cloud.doctype.proxy_server.proxy_server import (
			process_update_nginx_job_update,
		)
		from cloud.cloud.doctype.server.server import process_new_server_job_update
		from cloud.cloud.doctype.site.erpnext_site import (
			process_setup_erpnext_site_job_update,
		)
		from cloud.cloud.doctype.site.site import (
		...
		from cloud.cloud.doctype.site_backup.site_backup import process_backup_site_job_update
		from cloud.cloud.doctype.site_domain.site_domain import process_new_host_job_update
		from cloud.cloud.doctype.site_update.site_update import (
		...
		if site_migration and job_matches_site_migration(job, site_migration):
			...
			DeployCandidate.process_run_remote(job, response_data)

...
from cloud.cloud.doctype.app_source.app_source import AppSource, create_app_source
...
class ReleaseGroup(Document, TagHelpers):
	...
	if TYPE_CHECKING:
		...
		from cloud.cloud.doctype.common_site_config.common_site_config import CommonSiteConfig
		from cloud.cloud.doctype.release_group_app.release_group_app import ReleaseGroupApp
		from cloud.cloud.doctype.release_group_dependency.release_group_dependency import (
			ReleaseGroupDependency,
		)
		from cloud.cloud.doctype.release_group_mount.release_group_mount import (
			ReleaseGroupMount,
		)
		from cloud.cloud.doctype.release_group_package.release_group_package import (
			ReleaseGroupPackage,
		)
		from cloud.cloud.doctype.release_group_server.release_group_server import (
			ReleaseGroupServer,
		)
		from cloud.cloud.doctype.release_group_variable.release_group_variable import (
	...
	def deploy_in_progress(self):
		from cloud.cloud.doctype.deploy_candidate.deploy_candidate import (
			TRANSITORY_STATES as DC_TRANSITORY,
		)

		from cloud.cloud.doctype.bench.bench import TRANSITORY_STATES as BENCH_TRANSITORY

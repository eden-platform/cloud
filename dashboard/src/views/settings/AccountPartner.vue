<template>
	<Card
		title="Eden Partner"
		subtitle="Eden Partner associated with your account"
		...
		<div>
			...
			<div class="py-4">
				<h3 class="text-base text-gray-700" v-if="$account.parent_team">
					Only parent team can link with Eden Partner.
				</h3>
				<h3
					class="text-base text-gray-700"
					v-if="!$account.partner_email && !$account.parent_team"
				>
					Have a Eden Partner Referral Code? Click on
...
export default {
	...
	methods: {
		async referralCodeChange(e) {
			...
			let result = await this.$call('cloud.api.partner.validate_partner_code', {
				code: code
			});

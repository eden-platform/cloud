	...
	<WizardCard>
		<div>
			...
			<div class="space-y-8 sm:space-y-6" v-else>
				...
				<div>
					<label class="text-lg font-semibold"> Select a Eden version </label>
					<p class="text-base text-gray-700">
						Select a Eden version for your bench.
					</p>
					<FormControl
						class="mt-2"
						type="select"
...
export default {
	...
	watch: {
		selectedVersionName() {
			this.$nextTick(() => {
				let frappeApp = this.getEdenApp(this.selectedVersion.apps);
		...
		selectedApps: {
			handler(newVal, oldVal) {
				...
				let hasEden = newVal.find(app => app.app === 'frappe');
				if (!hasEden && oldVal) {

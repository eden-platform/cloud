<template>
	<Card
		...
		title="Eden Platform Partner"
		subtitle="Eden Platform Partner associated with your account"
	>
		<template #actions>
			<Button
		...
		<div class="py-4">
			<span
				...
				v-if="!$team.doc.partner_email"
			>
				Have a Eden Platform Partner Referral Code? Click on
...
export default {
	...
	resources: {
		addPartnerCode() {
			return {
				url: 'cloud.api.partner.add_partner',
				params: {
		...
		partnerName() {
			return {
				url: 'cloud.api.partner.get_partner_name',
				auto: true,
				params: {
	...
	methods: {
		async referralCodeChange(e) {
			...
			let result = await frappeRequest({
				url: 'cloud.api.partner.validate_partner_code',
				params: { code: code }
			});
